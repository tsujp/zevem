...
*** Opcodes
:LOGBOOK:
CLOCK: [2026-01-02 Fri 20:52]--[2026-01-02 Fri 22:59] =>  2:07
- Lot's of YP reading and notes (I am very tired). Got a plan for CREATE/2 now, got Korean tests next week but have an actionable plan for 'pausing' execution so that the host orchestrates the nested call and resumes.
CLOCK: [2025-12-18 Thu 22:43]--[2025-12-18 Thu 22:53] =>  0:10
- Add SELFBALANCE without tests
CLOCK: [2025-12-18 Thu 22:35]--[2025-12-18 Thu 22:42] =>  0:07
- Add CHAINID without tests
CLOCK: [2025-12-18 Thu 22:20]--[2025-12-18 Thu 22:32] =>  0:12
- Add ADDRESS without tests
...
Not comprehensive, specific final implementation details have their own TODOs.
...
**** TODO CREATE, CREATE2
:LOGBOOK:
CLOCK: [2026-02-15 Sun 18:37]--[2026-02-15 Sun 19:30] =>  0:53
- CREATE2 union tag assert, add stubbed CREATE
CLOCK: [2026-01-14 Wed 21:01]--[2026-01-14 Wed 21:50] =>  0:49
- A bit of CREATE2 implementation and yak shaving on representation requirement for host to orchestrate. Will just have a field on EVM struct and go that route for now versus tagged error union return. Still need to do gas (326) for CREATE2 and then finish the test scenario for resumption.
- State "TODO"       from              [2026-02-15 Sun 19:20] \\
  Various things like exact gas costs (G_initcodeword), and their entries from H.1(326) are still to-do as well as tests asserting correct VM state afterwards. Left for now as I await project status and until the rest of the opcodes are generally done.
:END:

- [ ] H.1(326) behaviour
- [ ] G fee schedule minutia (e.g. G_initcodeword)

**** TODO LOG
:LOGBOOK:
CLOCK: [2026-02-15 Sun 19:37]--[2026-02-15 Sun 21:18] =>  1:41
- LOG notes and skeleton implementation, design decision around where log entries should live (client or zevem) and so until that is decided only pulling items off the stack is implemented.
- State "TODO"       from              [2026-02-15 Sun 21:14] \\
  LOG has some design decisions that block it from complete implementation at this time.
:END:

- [ ] Who owns LOG entry storage, client or zevem? Once known placement of log topic appending can be implemented and then actual tests (and proper behaviour) of LOG can be tested.
- [ ] G_log
- [ ] G_logdata
- [ ] G_logtopic
- [ ] H.1(326) behaviour
- [ ] Gas pricing tests, LOG also touches memory

*** TODO Precompiles
:LOGBOOK:
- State "TODO"       from              [2026-02-15 Sun 21:24] \\
  Precompile contracts need to be added.
:END:
...
*** Misc
:LOGBOOK:
CLOCK: [2026-01-02 Fri 20:25]--[2026-01-02 Fri 20:51] =>  0:26
- Looking at JSON test fixtures to incorporate into Zevem; holding off for now since I think a few more opcodes can still take unit tests meaningfully.
